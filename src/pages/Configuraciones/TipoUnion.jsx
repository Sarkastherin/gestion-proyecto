...
export default function TipoUnion() {
  const { listaUnion, postData, getListaUnion } = useMateriales();
  const {
    register,
    handleSubmit,
    reset,
  ...
  return (
    <LayoutConfiguraciones
      columns={columns}
      data={listaUnion}
      title={"Crear nuevo Tipo de Unión"}
      form={<Form />}
      handleSubmit={handleSubmit}
      onSubmit={onSubmit}
      onError={onError}
      state={state}
      color={{color: "text-emerald-500", variant: "emerald"}}
    />
  );
}
